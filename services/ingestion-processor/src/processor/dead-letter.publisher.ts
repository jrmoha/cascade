...
/**
 * Publishes messages that failed downstream processing to the dead-letter topic
 * (`raw-events.dlq`, KAN-23). The message is validated against the shared
 * `deadLetterSchema` before it is produced, so the DLQ only ever holds
 * well-formed, replayable records.
 *
